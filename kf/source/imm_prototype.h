...
class IMM_prototype : public Filter<M>
{
private:
    M state;
    M covariance;
...
public:
    ...
    IMM_prototype(E1 e1, E2 e2, E2 e3, M m, M t):
        estimator1(e1),
        estimator2(e2),
        estimator3(e3),
        mu(m),
    ...
    IMM_prototype(M state,M covariance,M process_noise,M measureNoise, M m, M t):
    ...
    M mixing_probability()
    {
        ...
        //std::cout << "imm.ret.mx: " << std::endl << mx << std::endl;
    ...
    double system_probability(M& z,M& ze,M& Se)
    {
        ...
        //return std::pow((1./2./M_PI),(n/2))/std::sqrt(Se.determinant())*std::exp(power.determinant());
        return (1./std::sqrt(std::pow(2.*M_PI,n)*Se.determinant()))*std::exp(power.determinant());
    }
    ...
    M models_probability_recalculation(M& z, M& ze1, M& ze2, M& ze3, M& Se1, M& Se2, M& Se3)
    {
        ...
        long double mu0a = system_probability(z,ze1,Se1)*(mx(0,0)*mu(0)+mx(1,0)*mu(1)+mx(2,0)*mu(2));
        long double mu1a = system_probability(z,ze2,Se2)*(mx(0,1)*mu(0)+mx(1,1)*mu(1)+mx(2,1)*mu(2));
        long double mu2a = system_probability(z,ze3,Se3)*(mx(0,2)*mu(0)+mx(1,2)*mu(1)+mx(2,2)*mu(2));

        long double denominator = mu0a+mu1a+mu2a;

        long double mu0 = mu0a/denominator;
        long double mu1 = mu1a/denominator;
        long double mu2 = mu2a/denominator;
    ...
    std::pair<M,M> predict(double dt)
    {
        ...
        //std::cout << "imm.mxs.X01: " << std::endl << mxs.X01 << std::endl;
        //std::cout << "imm.mxs.P01: " << std::endl << mxs.P01 << std::endl;
        ...
        //std::cout << "imm.predict.state: " << std::endl << state << std::endl;
        //std::cout << "imm.predict.covariance: " << std::endl << covariance << std::endl;
    ...
    std::pair<M,M> correct(const M& measurement)
    {
        ...
        //std::cout << "imm.correct.state: " << std::endl << state << std::endl;
        //std::cout << "imm.correct.covariance: " << std::endl << covariance << std::endl;
...
public:
    ...
    M getMeasurementPredict()const override{return (estimator1.getMeasurementPredict()+ //#ZAGL
                                           estimator2.getMeasurementPredict()+
                                           estimator3.getMeasurementPredict())/3.;}
    M getCovarianceOfMeasurementPredict()const override{return (estimator1.getCovarianceOfMeasurementPredict()+ //#ZAGL
                                                       estimator2.getCovarianceOfMeasurementPredict()+
                                                       estimator3.getCovarianceOfMeasurementPredict())/3.;}
